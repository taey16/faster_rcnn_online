...
NETS = {'vgg16': ('VGG16',
                  '/usrdata2/workspace/faster_rcnn_online/output/eleven_all_vgg16_scale_jitter/eleven_all_train/eleven_all_vgg16_faster_rcnn_anneal_stepsize200000_iter_1600000.caffemodel'),
        'res50': ('RES50',
                  '/usrdata2/workspace/faster_rcnn_online/output/eleven_all_res50_scale_jitter/eleven_all_train/eleven_all_ResNet-50_faster_rcnn_anneal_stepsize450000_iter_1600000.caffemodel')}
...
if __name__ == '__main__':
    ...
    if args.image is None :
	    print args.thres
	    metafile = os.path.join('/', 'usrdata', 'ImageSearch', '11st_DB', '11st_All', 'Annotations', 'annotations_test.txt')
	    total_precision = 0
	    total_recall = 0
	    num_query = 0
		...
		for line in f:
			...
			for i in range(0, num_rois):
				...
				box = [ int(word[step+3]), int(word[step+4]), int(word[step+5]), int(word[step+6]) ]
				box = np.array(box)
				gt_boxes.append(box)		
		
	                im_path = os.path.join('/', 'usrdata', 'ImageSearch', '11st_DB', '11st_All', 'Images', image_name + '.jpg')
		        print im_path
		        pred_clses, pred_boxes = detect(net, im_path)
			precision, recall = calc_precision(gt_boxes, gt_clses, pred_boxes, pred_clses, args.thres)
			total_precision += precision
			total_recall += recall
			num_query += 1
			print 'total_precision : %f, total_recall : %f, num_query : %d'%(total_precision, total_recall, num_query)

	    print 'precision : %f, recall : %f'%((float(total_precision) / num_query) * 100, (float(total_recall) / num_query) * 100)

    else :
	im_path = os.path.join('/', 'usrdata', 'ImageSearch', '11st_DB', '11st_All', 'Images', args.image + '.jpg')
	print im_path
	demo(net, im_path)
	plt.show()
